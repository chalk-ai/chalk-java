...
public class TestFeather {
    ...
    public void testConvertBytesResponseToResult() throws Exception {
        String encodedString = new String(Files.readAllBytes(Paths.get(System.getProperty("user.dir"), "src/test/java/chalk/arrow/", "bulk_query_response.txt")), "UTF-8");
    ...
    public void testMillionRowsInOutput() {
        try {
            byte[] bytes = Files.readAllBytes(Paths.get(System.getProperty("user.dir"), "src/test/java/chalk/arrow/", "million_scalar_rows.bin"));
    ...
    public void testListsInScalarTable() throws Exception {
        byte[] bytes = Files.readAllBytes(Paths.get(System.getProperty("user.dir"), "src/test/java/chalk/arrow/", "lists_in_scalar_table.bin"));
        OnlineQueryBulkResponse response = OnlineQueryBulkResponse.fromBytes(bytes);
        OnlineQueryResult result = response.toResult();
        Table scalarsTable = result.getScalarsTable();
        assert scalarsTable.getRowCount() == 5;
    ...
    public void testStructsInScalarTable() throws Exception {
        byte[] bytes = Files.readAllBytes(Paths.get(System.getProperty("user.dir"), "src/test/java/chalk/arrow/", "structs_in_scalar_table.bin"));
        OnlineQueryBulkResponse response = OnlineQueryBulkResponse.fromBytes(bytes);
        OnlineQueryResult result = response.toResult();
        Table scalarsTable = result.getScalarsTable();
        assert scalarsTable.getRowCount() == 5;
    ...
    public void testPrimitiveTypesInOutputTable() throws Exception {
        byte[] bytes = Files.readAllBytes(Paths.get(System.getProperty("user.dir"), "src/test/java/chalk/arrow/", "all_types_in_scalar_table.bin"));
        OnlineQueryBulkResponse response = OnlineQueryBulkResponse.fromBytes(bytes);
        OnlineQueryResult result = response.toResult();
        Table scalarsTable = result.getScalarsTable();
